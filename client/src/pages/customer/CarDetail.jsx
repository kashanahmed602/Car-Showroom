...
const PublicCarDetail = () => {
  ...
  const fetchCar = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/public/cars/${id}`);
  ...
  const handleInquiry = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/public/inquiry`, {
        ...inquiryForm,
        carId: id
      });
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/public/wishlist`, {
        carId: id,
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
                    {car.images.map((image, index) => (
                      <button
                        ...
                        className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-pointer hover:scale-105 ${selectedImageIndex === index
          ...
          <div className="space-y-6">
            <div className="animate-slide-in">
              ...
              <div className="flex items-baseline gap-4 mb-8">
                ...
                <span className={`px-4 py-1.5 rounded-full text-sm font-semibold border ${car.status === 'AVAILABLE'
            ...
            <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-2 max-w-4xl overflow-x-auto px-4">
              {car.images.map((image, index) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${lightboxImageIndex === index
                    ? 'border-cyan-500 ring-2 ring-cyan-500/50 shadow-lg shadow-cyan-500/30'
